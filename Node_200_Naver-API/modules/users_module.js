...
export const userJoin = async (user) => {
  ...
  try {
    ...
    console.log(e);
    throw new Error(SYSTEM_RES.SQL_ERROR);
  }
  if (resultUser) throw new Error(JSON.stringify(USER_RES.ORVERLAP_USERNAME));
...
export const userLogin = async (user) => {
  ...
  if (!findUser) throw new Error(JSON.stringify(USER_RES.MATCH_NOT_USERNAME));
  ...
  if (encPassword !== findUser.password)
    throw new Error(JSON.stringify(USER_RES.MATCH_NOT_PASSWORD));
  return findUser;
};
